...
# Explicitly exclude Arabic enumeration list items such as "1、xxx", "2.xxx".
...
def detect_level(text: str) -> Tuple[str, float, List[str]]:
    ...
    if ARABIC_ENUM_RE.match(text):
        reasons.append("exclude:arabic_enum")
...
def iter_candidates(payload: Dict[str, Any]) -> Tuple[List[TitleCandidate], Dict[str, int]]:
    ...
    for page in payload.get("pdf_info", []) or []:
        ...
        for block in page.get("para_blocks", []) or []:
            ...
            if level == "OTHER":
                if "exclude:arabic_enum" in reasons:
                    stats["excluded_arabic_enum"] += 1
                else:
...
def main() -> int:
    ...
    summary_path = out_dir / "mineru_heading_hierarchy_summary.txt"
    tree_txt_path = out_dir / "mineru_heading_hierarchy_tree.txt"
    tree_json_path = out_dir / "mineru_heading_hierarchy_tree.json"
    candidates_tsv_path = out_dir / "mineru_heading_hierarchy_candidates.tsv"

    l1_count = sum(1 for c in high_conf if c.level == "L1")
    l2_count = sum(1 for c in high_conf if c.level == "L2")

    summary_lines = [
        "High-confidence hierarchy recovery summary (L1/L2 only)",
        f"source={source_json}",
        f"total_title_blocks={stats['total_title_blocks']}",
        f"noise_filtered={stats['noise_filtered']}",
        f"excluded_arabic_enum={stats['excluded_arabic_enum']}",
        ...
        "rule=type:title + regex(L1/L2-cn) + bbox layout; arabic enum excluded",
    ]
    ...
    print(f"excluded_arabic_enum={stats['excluded_arabic_enum']}")
    print(f"high_conf_candidates={len(high_conf)}")
    print(f"L1={l1_count}, L2={l2_count}")
    return 0
