...
class DocumentMapper:
    ...
    async def upsert_documents(self, documents: Iterable[Document]) -> int:
        ...
        if self.backend == "local_dev":
            ...
            return await asyncio.to_thread(self.local_repository.upsert_documents, records)
    ...
    async def list_by_collection(self, collection_name: str, limit: int = 1000) -> list[Document]:
        if self.backend == "local_dev":
            assert self.local_repository is not None
            return await asyncio.to_thread(self.local_repository.list_documents_by_collection, collection_name, limit)
    ...
    async def get_by_doc_id(self, doc_id: str) -> Optional[Document]:
        if self.backend == "local_dev":
            ...
            return await asyncio.to_thread(_get_local)
        ...
        async with get_async_session(backend="pgvector", database_url=self.database_url) as session:
            row = (await session.execute(query, {"doc_id": doc_id})).mappings().first()
